...
4. The **Job details** page opens. In-case the job to not completed, refresh the status using the refresh button (A). Review the job **Status** change from **Running** to either **completed** or **completed with warnings**.  Click on the latest timestamp of the job run (B) to view the details.<br><br>
    ...
    ![screenshot](./attachments/23_ls.png) <br><br>

    ⚠️ Note: The metadata enrichment process can occasionally take some time to complete. If it takes too much time (more than 5 mins), You can skip this process by importing an already enriched project - Proceed with [this lab guide](https://github.ibm.com/AmericasTopTeam/wxbi-client-bootcamp/blob/main/Labs/Lab2-data_steward_experience/README_with_cooked_mde.md#14-perform-metadata-enrichment), start from step 5 in this sub-section. <br><br>

5. The "About this run" screen provides information about the enrichment job, including details about the **Objectives** (A) of the run and asset failures along with other details. Click the **Log** tab (B) to view the detailed job log.<br><br>
...
9.  When you see the validation results as *The expression is valid* (D), click the blue **Ok** button (E). <br><br>
    ...
    ![screenshot](./attachments/35_ls.png) <br><br>
...
10.  When you see the validation results as *The expression is valid* (A), click the blue **Ok** button (B). <br><br>![screenshot](./attachments/35_ls_3.png) <br><br>The next step is to build metrics definitions.
...
9.  The visualization is added to the Metric overview. Click on the 3-dot ellipsis beside the **Actual Revenue** metric that you created previously. (A) <br><br>
