...
14. Again, at the top of the **sample_hr** schema, search for **sls** (A) and select tables:
    - `sls_order_method_dim`, `sls_product_brand_lookup`, `sls_product_color_lookup`, `sls_product_dim`, `sls_product_line_lookup`, `sls_product_lookup` (B), 
    - `sls_product_type_lookup`, `sls_rtl_dim`, `sls_sales_fact` (C) <br>
...
16. Ensure 15 assets are selected by reviewing **selected assets (15/15)** (A) and Click the blue **Add** button (B). 
    - Note: you may need to scroll to the right.  <br><br>

    ![screenshot](./attachments/16.png) <br><br>

17. The **Select data** window appears. The data stored in the selected tables can be previewed. Click on the 3-dot ellipsis on the right-side of the **emp_employee_dim** table (A) and **Preview** (B) the data. <br><br>

    ![screenshot](./attachments/17.png) <br><br>

    A sample of the first 100 rows of data stored in the **emp_employee_dim** table is displayed to determine if the correct table is selected to define the metrics. <br><br>

18. Click the **‘x’** button in the upper-right of the interface to close the preview (A). <br><br>
    
    ![screenshot](./attachments/18.png) <br><br>

19. From the **Select data** screen, click the blue **Next** button (A). <br><br>
...
5. The "About the run" screen provides information about the enrichment job, including details about the **Objectives** (A) of the run and asset failures along with other details. Click the **Log** tab (B) to view the detailed job log.<br><br>
    
    ![screenshot](./attachments/24.png) <br><br>

6. In this example, scrolling down the log shows a non-fatal asset issues (A). These is relate to tables which does not have string columns, so they can be ignored. <br><br>
    
    ![screenshot](./attachments/25.png) <br><br>

7. Close the **Job run details** browser tab to return to the **Enrich data** browser tab. Click the **Review enriched data** link (A). <br><br>
    
    ![screenshot](./attachments/26.png) <br><br>

8. The enriched metadata opens in a new browser tab. Scroll to the right to view the **display names** that were generated for each table (A), as well as **suggested table descriptions** (B) and **business terms** (C).   The best practice is to go through every row in this table and make sure the AI-generated display names, descriptions, and business terms are accurate and consistent with the way the enterprise defines or describes. <br><br>
    
    ![screenshot](./attachments/27.png) <br><br>

9. Click on the : 
    - **go_region_dim** row (A) to evaluate the AI-generated table description.
...
10. Once the AI suggested descrptions are accepted, that become the actual description (A) for that row. <br><br>
    
11. To apply any updates to the metadata, the enrichment job must be run by clicking the blue **Enrich all assets** button - not required for the lab. <br><br>
    
    ![screenshot](./attachments/29.png) <br><br>

12. Close the **Sales Model** browser tab to return to the Enrich data browser tab, and then click the blue **Next** button (A). <br><br>
...
**Note** - It is fine in-case the semantic model representation and the generated metrics definition names differs from the screenshots below.

### 2.1 Manually create metrics

1. watsonx BI provides flexibility to either generate the metrics automatically or build the metrics manually. Click the **Build metrics in advanced mode** tile (A) to start building metrics manually. <br><br>
    ...
    IBM watsonx BI has leveraged defined relationships within the data source and the result is a semantic model with enterprise-grade modeling capabilities, including the ability to create calculations, modify relationships, define filters, etc. The initial view presents all relationships in the database. Notice how the tables reflect the generated display names. <br><br>
    
    ![screenshot](./attachments/32.png) <br><br>
...
3. Toggle **Focus mode** (B) to disable focus mode at any time to show all relationships. <br><br>
    ...
    ![screenshot](./attachments/33.png) <br><br>
...
5. Review the modeling options available, and then select Calculation (B). <br><br>
    ...
    ![screenshot](./attachments/34.png) <br><br>
...
9.  When you see the validation results as *The expression is valid* (D), click the blue **Ok** button (E). <br><br>
    
    ![screenshot](./attachments/35.png) <br><br>
...
3. Click **Metric definition** from the context menu (C). <br><br>
    ...
    ![screenshot](./attachments/36.png) <br><br>

4. The **Metric definition** window appears. Enter **Actual Revenue** in the Label text box (A). <br><br>
    
    ![screenshot](./attachments/37.png) <br><br>

5. Click the **Data scope** tab from the left navigation menu (A). <br><br>
   
6. Expand the tables to see all columns that have been automatically added to the data scope for **Actual Revenue** metric (B). <br><br>

7. Click the blue **Done** button (C). <br><br>
    
    ![screenshot](./attachments/38.png) <br><br>
...
9. Select **Generate metric definitions** from the context menu (B). <br><br>
    ...
    ![screenshot](./attachments/39.png) <br><br>
...
13. Select **Save** from the drop-down menu (B). <br><br>
    ...
    ![screenshot](./attachments/41.png) <br><br>

14. When the model has completed saving, you should see a green message indicating that it has saved successfully (A). <br><br>
    
    ![screenshot](./attachments/42.png) <br><br>

15. Hover over the **Actual Revenue** metric that you created and click on the 3-dot ellipsis that appears (A). <br><br>
...
18. Repeat steps 15 and 16 to export the metrics definitions for the remaining metrics (D) in your semantic model - the metrics names might not look the same as shown and that's okay.  <br><br>
    ...
    ![screenshot](./attachments/43.png) <br><br>
...
1. Click on the **Metrics overview** link (A). <br><br>
    ...
    ![screenshot](./attachments/43.1.png) <br><br>

2. You are provided with an overview of the generated metrics, the ability to generate visualizations (or build them manually), and a method for publishing the metrics to your users. When all of the metrics have completed their enrichment process and have a status of **Ready** (A) click on the **Generate visualizations** link from the toolbar (B). <br><br>
    
    ![screenshot](./attachments/44.png) <br><br>

3. The **Generate visualizations** window appears. Multi-select all the metrics by checking their associated checkboxes (A). <br><br>
   
4. Click the blue **Next** button (B). <br><br>
    
    ![screenshot](./attachments/45.png) <br><br>

5. IBM watsonx BI has created 5 different visualizations from the data in the scope of each metric definition (A). <br><br>

6. Check one of the visuals from **Actual Revenue** metric definition visualization (B). <br><br>
   
7. Click the blue **Add** button (C). <br><br>
    
    ![screenshot](./attachments/46.png) <br><br>

8. The visualization is added to the Metric overview. Click on the 3-dot ellipsis beside the **Actual Revenue** metric that you created previously. (A) <br><br>
   
9. Select **Build visualizations** from the context menu (B). <br><br>
    
    ![screenshot](./attachments/47.png) <br><br>

10. The **Build visualization** window appears. From the gallery of available visualizations, scroll down and click the **Stacked column** chart type (A). <br><br>
    
11. Click the **Chart** fields tab (B). <br><br>
    
    ![screenshot](./attachments/48.png) <br><br>

12. Click the **Add field** button below **Categories** and select **Order Method Description - English** (A) or similar field. <br><br>
    
13. Click the **Add field** button below **Values** and select **Revenue** (B). <br><br>
    
14. Click the **Add field** button below **Color** and select **City** (C). <br><br>
    
15. Click the **Visualization details** tab (D). <br><br>
    
    ![screenshot](./attachments/49.png) <br><br>

16. Under **Visualization name** enter **Revenue by Order Method** (A). <br><br>
    
17. Under **Chart title** enter **Revenue by Order Method** (B). <br><br>
    
18. Select the **Chart properties** tab (C). <br><br>
    
    ![screenshot](./attachments/50.png) <br><br>

19. Here you can control and modify the chart properties to align with the needs of your business, including color palettes, axis labels and colors, or fill colors. Expand the Chart drop-down (A) and click the **Show legend** toggle to enable/disable the legend (B). <br><br>
    
20. The chart is interactive, so if you hover over a value you can see the related details, making the legend less useful (C). <br><br>

21.  Click the blue **Save** button (D). <br><br>
    
        ![screenshot](./attachments/51.png) <br><br>
...
1. Click the **Publish** link (A). <br><br>
    ...
    ![screenshot](./attachments/52.png) <br><br>

2. The **Publish** window appears in the **Select metrics** section. From the **Select the metrics and visualizations that you want to publish** area, check the metrics with visualizations that you’ve created (**Actual Revenue**) by checking the checkbox within the visualization (A). <br><br>
   
3. Click the blue **Next** button (B). <br><br>
    
    ![screenshot](./attachments/53.png) <br><br>
...
10. Click on the **Actual Revenue** metric (C). <br><br>
    
    ![screenshot](./attachments/55.1.png) <br><br>
...
12. Click the blue **Pin to Key metrics** button (A) <br><br>
    ...
    ![screenshot](./attachments/55.3.png) <br>

With the metrics successfully published to the catalog and pinned in the **Key metrics** section, you can see and interact with it through watsonx BI’s conversational interface. <br>

![screenshot](./attachments/55.4.png) <br><br>

### 2.5 Sample Questions to Test

1. Show me the total revenue by year.
2. Which product line generated the highest revenue in 2024?
3. What was the total quantity sold for each product type in 2024?
4. Show me the gross profit by retailer country for 2024.
5. Which order method type had the highest planned revenue in 2024?
6. How many units of each product were sold by retailers in California?
7. Compare average unit sale price between different retailer states.
8. Can you provide the cost of goods sold by product type?
9. Can you show the top 5 products by revenue in the United States?
10. Which retailer country had the highest average gross profit?
11. What is the planned revenue versus actual revenue for each year?
12. What is the revenue and gross profit for each product line in the USA for 2024?
13. What imapacts revenue?
